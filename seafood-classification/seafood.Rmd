...
#create document frequency matrix with only Noun
...
set.seed(1234)
knn.result.noun <- knn(train.dfm,test.dfm,train.dfm$label, k = 1, use.all = F )
best.accuracy(knn.result.noun,testdf = test.dfm)
system("say just finished")
...
knn.result.svd <- vector(mode = "list",length=1)
tune.knn <- c(500,1000) #low k might be overfit models

set.seed(1234)
for (i in seq_along(tune.knn)) {
  knn.result.svd[[i]] <- knn(train.svd,test.svd,train.svd$label, k = tune.knn[i])
}
...
test.predicted.actual.logsvd <- data.frame(name = test$name,
                         ...
                         predicted = log.result.svd[[1]]) ## best threshold is 0.01
...
total.time.train.log.noun <- Sys.time() - start.time
...
test.predicted.actual.lognoun <- data.frame(name = test$name,
                         translate = test$translatedText,
                         actual = test$seafood_yn,
                         predicted = log.result.noun[[1]]) ## best threshold is 0.33

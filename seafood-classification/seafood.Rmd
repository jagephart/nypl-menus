...
```{r}
train.tfidf <- data.frame(label = as.factor(train$seafood_yn), dfm.train.tfidf)
test.tfidf <- data.frame(label = as.factor(test$seafood_yn), dfm.test.tfidf)

set.seed(1234)
# use caret package to make stratified 10-fold cross validation repeated
# use caret package to make stratified 10-fold cross validation repeated

cv.folds <- createMultiFolds(train.tfidf$label, k = 10, times = 2)
cv.control <- trainControl(method = "repeatedcv", number = 10,
                           repeats = 2, index = cv.folds)
...
allvar.logFit <- train(label ~ ., data = train.tfidf, 
                method = "glm", 
                family=binomial(),
                trControl = cv.control, 
                tuneLength = 10)

total.time.train.log.allvar <- Sys.time() - start.time

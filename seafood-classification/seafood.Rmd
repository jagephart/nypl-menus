...
```{r}
#load dataset
df <- read_csv("Seafood_Dishes.csv")
...
Q: # do we need to translate? 
```{r}
#detect langauage -- cld3 didn't return appropriate detection
df.labeled.cld3 <- df%>%
  select(-description) %>%
  filter(seafood_yn %in% c(0,1)) %>%
...
df.labeled.cld3 %>%
  filter(lang == "french")
unique(df.labeled.cld3$lang)
#detect langauage -- textcat didn't return appropriate detection

df.labeled.textcat <- df%>%
  select(-description) %>%
  filter(seafood_yn %in% c(0,1)) %>%
  mutate(name = str_to_lower(name),
         lang = textcat::textcat(name))
df.labeled.textcat %>%
  filter(lang == "french")
unique(df.labeled.textcat$lang)

# should we use Google API?

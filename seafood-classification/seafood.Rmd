...
test.predicted.actual.logsvd <- data.frame(name = test$name,
                         translate = test$translatedText,
                         actual = test$seafood_yn,
                         predicted = log.result.svd[[40]]) ## best threshold is 0.01
...
test.predicted.actual.logsvd.2 <- data.frame(name = test$name,
                         translate = test$translatedText,
                         actual = test$seafood_yn,
                         predicted = log.result.svd.2[[31]]) ## best threshold is 0.01
...
test.predicted.actual.lognoun <- data.frame(name = test$name,
                         translate = test$translatedText,
                         actual = test$seafood_yn,
                         predicted = log.result.noun[[27]]) ## best threshold is 0.27 #
...
##################### predict with test ##################
# predict
logpredict <- predict(logfit.noun, newdata = test.dfm.allword,  type = "response")
...
## make .csv for predicted != actual to check

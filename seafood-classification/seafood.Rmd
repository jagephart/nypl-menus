...
tune.knn <- c(3,5,25,50,75)
library(class)

set.seed(1234)
for (i in seq_along(tune.knn)) {
  knn.result[[i]] <- knn(train.svd,test.svd,train.svd$label, tune.knn[i])
}

## create a function to determine best model
best.accuracy <- function(result) {
  accuracy <- 1:length(result)
  n.test <- nrow(test.svd)
  for (i in seq_along(result)) {
    accuracy[[i]] <- sum(test.svd$label == result[[i]] )/n.test 
  }
  CPR <- max(accuracy)
  k <- which.max(accuracy)
  conf.matrix <- confusionMatrix(as.factor(result[[k]]),test.svd$label)
...
## use logistic regression with svd
